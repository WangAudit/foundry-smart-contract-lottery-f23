...
contract CreateSubscription is Script {
    ...
    function createSubscription(address vrfCoordinator, uint256 deployerKey) public returns (uint64) {
        console.log("Creating subscription of a Chain: ", block.chainid);
...
contract FundSubscription is Script {
    uint96 public constant FUND_ACCOUNT = 3 ether;
    ...
    function fundSubscription(
        ...
        if (block.chainid == 31337) {
            vm.startBroadcast(deployerKey);
            VRFCoordinatorV2Mock(vrfCoordinator).fundSubscription(subId, FUND_ACCOUNT);
            vm.stopBroadcast();
        } else {
            vm.startBroadcast();
            LinkToken(link).transferAndCall(vrfCoordinator, FUND_ACCOUNT, abi.encode(subId));
            vm.stopBroadcast();
        }
    }
